...
class TestUser(unittest.TestCase):
    ...
    def setUp(self):
        self.test_User = User("Yusuf", "yswef1234", "5:22")
    ...
    def test_create_task(self, mock_input, mock_open):
        self.test_User.create_task()
        task_deadlien = datetime.strptime("2024-12-31", "%Y-%m-%d").date()
        self.assertIn("Complete the project", self.test_User.task_list)
        self.assertEqual(
            self.test_User.task_list["Complete the project"]["description"],
            "Project description",
        )
        self.assertEqual(
            self.test_User.task_list["Complete the project"]["deadline"],
            f"{task_deadlien}",
        )
        self.assertEqual(
            self.test_User.task_list["Complete the project"]["priority"], "Red"
        )

    def test_log_phone_usage(self):
        with patch("sys.stdout", new=StringIO()) as fake_out:
            self.test_User.log_phone_usage()
    ...
    def test_password_check_success(self, mock_input, mock_open):
        a = self.test_User._password_check()
        self.assertEqual(
            a, "yswef1234", "Password check did not return True for a strong password"
        )
    ...
    def test_password_check_failure(self, mock_input):
        a = self.test_User._password_check()
        self.assertEqual(
            a, False, "Password check did not return False for a weak password"
        )

    def test_view_tasks_with_tasks(self):
        self.test_User.task_list = {
            "Task 1": {
